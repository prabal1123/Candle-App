...
export default function CheckoutScreen() {
  ...
  // ✅ useCart gives you an ARRAY of items + a memoized subtotal (in rupees)
  const { items, subtotal, clear: clearCart } = useCart();

  // Local checkout form state
  const [name, setName] = useState("");
  const [phone, setPhone] = useState("");
  const [address, setAddress] = useState("");
  const [deliveryNotes, setDeliveryNotes] = useState("");
  ...
  const prepareOrder = async () => {
    ...
    try {
      ...
      const orderItems = items.map((it) => {
        ...
        const priceCents = toPaise(Number(it.price ?? 0)); // rupees -> paise
        return {
          product_id: it.id,
          name: it.title,      // normalized title from cart
          qty,
          price_cents: priceCents,
          price: paiseToRupees(priceCents), // rupees (display)
          sku: it.sku ?? undefined,
        };
      });

      // Totals (in paise)
      const subtotalPaiseLocal = orderItems.reduce((sum: number, li: any) => sum + (li.price_cents ?? 0) * (li.qty ?? 1), 0);
      const totalPaise = subtotalPaiseLocal; // add taxes/shipping here if needed
      ...
      <View style={{ marginTop: 18 }}>
        {!localOrder ? (
          ...
          <View>
            ...
            {/* Your existing payment component */}
            <PlaceOrderButton2
              localOrder={localOrder}
              backendUrl={`${API_BASE}/api`}
              onPaid={() => {
