...
import supabase from "@/lib/supabase";
import type { AppDispatch } from "@/store";
import { addToCart, removeFromCart, updateQuantity, clearCart, loadCart } from "@/features/cart/cartSlice";
...
/** Add or increment an item in the remote cart */
export async function syncAddItem(
  dispatch: AppDispatch,
  ...
  try {
    ...
    dispatch(loadCart(identity));
  } catch (err) {
    console.error("[cartSync] syncAddItem failed:", err);
    // don’t reload on error → preserve optimistic state
  }
}
...
export async function syncUpdateQuantity(
  ...
  try {
    ...
    if (newQty <= 0) {
      await supabase.from("cart_items").delete().match({ cart_id: cartId, product_id: productId });
      dispatch(loadCart(identity));
      return;
    }
