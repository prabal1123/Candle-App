  ...
  return (
    <View style={styles.root}>
      {items.map((item) => (
        <View key={item.key} style={styles.row}>
          ...
          <View style={styles.meta}>
            ...
            <View style={styles.controlsRow}>
              <Pressable onPress={() => updateQty(item.key, Math.max(0, item.qty - 1))} style={styles.qtyBtn}>
                <Text>-</Text>
              </Pressable>

              <Text style={styles.qtyText}>{item.qty}</Text>

              <Pressable onPress={() => updateQty(item.key, item.qty + 1)} style={styles.qtyBtn}>
                <Text>+</Text>
              </Pressable>

              <Pressable onPress={() => remove(item.key)} style={styles.removeBtn}>
                <Text style={styles.removeText}>Remove</Text>
              </Pressable>
            </View>
          </View>
      ...
      <View style={{ marginTop: 12 }}>
        <Text style={{ fontWeight: "700" }}>Subtotal: ₹{Number(subtotal).toFixed(2)}</Text>
      </View>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  price: { color: "#666", marginTop: 6 },
  controlsRow: { flexDirection: "row", alignItems: "center", marginTop: 10 },
  qtyBtn: { paddingHorizontal: 10, paddingVertical: 8, borderWidth: 1, borderColor: "#e5e7eb", borderRadius: 8 },
  qtyText: { marginHorizontal: 10, fontWeight: "700" },
  removeBtn: { marginLeft: 12 },
  removeText: { color: "#d04646", fontWeight: "600" },
});
