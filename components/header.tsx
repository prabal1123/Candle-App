...
// 👇 NEW: pull cart count from Redux
...
export default function Header() {
  ...
  // 👇 Get total items (sum of quantities; falls back to 1 per item)
  const cartCount = useSelector((s: RootState) => {
    const raw = (s as any)?.cart?.items;
    const items = Array.isArray(raw) ? raw : raw ? Object.values(raw as any) : [];
    return items.reduce((n: number, it: any) => n + (Number(it?.quantity) || 1), 0);
  });
  ...
  const menuHeight = anim.interpolate({ inputRange: [0, 1], outputRange: [0, 240] });
  ...
  // Small badge styles (inline so you don't need to edit headerStyles)
  const badgeWrapStyle = { position: "relative" as const };
  const badgeStyle = {
  ...
  const badgeTextStyle = { color: "#fff", fontSize: 11, fontWeight: "700" as const };
...
<Image 
  source={{ uri: "https://bybxickqlfiirrjkvuoq.supabase.co/storage/v1/object/public/Product%20List/e9a7f444-4b9d-497c-b225-29689963cb84.png" }} 
  style={[headerStyles.logo, { width: 60, height: 60 }]} // Manually overriding the size
  resizeMode="contain"
/>
